...
def generate_text_answer(df, question):
    ...
    # Special case: least prescribed
    if "least prescribed" in question_lower:
        if "number_of_prescriptions" in df.columns:
            df_filtered = df[["product_name", "number_of_prescriptions"]].dropna()
            least = df_filtered.sort_values("number_of_prescriptions").head(5)
            return "\n".join([
                f"{row['product_name']}: {int(row['number_of_prescriptions'])} prescriptions"
