                ...
                try:
                    ...
                    if isinstance(result, dict):
                        if any(word in user_input.lower() for word in ["chart", "visual", "bar", "graph"]):
                            ...
                            st.session_state.chat_history.append({"role": "assistant", "content": pio.to_json(fig)})
                        else:
                            formatted = "\n".join([
...
for msg in st.session_state.chat_history:
    if msg["role"] == "user":
        ...
        try:
            chart_obj = pio.from_json(msg["content"])
            st.plotly_chart(chart_obj, use_container_width=True)
        except:
            st.markdown(f'<div class="assistant-msg">[Chart failed to render]</div>', unsafe_allow_html=True)
